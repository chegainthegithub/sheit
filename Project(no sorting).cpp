...
vector<int> the_arr(int All_Goods, int num, int Number_Trades,char *name)
{
	...
	for (int i = 1; i <= Number_Trades; i++) //это происходит количество-сделок-раз(перебор всех сделок)
	{
		...
		int count = 0;
		for (int j = 0; j < limit; j++)   //выбор строки из локальной матрицы
		{
			for (int count = 0; count < Great_limit; count++) // выбор строки из общей матрицы
			{
				int local_counter = 0;
				int Big_size = Matrix_full[count].size();
				int Little_size = Matrix_local[j].size();
				for (unsigned int loc1 = 0; loc1 < Little_size; loc1++) // выбор элемента и локальной строки
				{
					for (unsigned int loc2 = 0; loc2 < Big_size; loc2++) // выбор элемента из общей строки
						if (Matrix_local[j][loc1] == Matrix_full[count][loc2])
						{
						local_counter++;
						break;
						}
					if (local_counter != loc1 + 1)
					{
						local_counter = 0;
						break;
					}
